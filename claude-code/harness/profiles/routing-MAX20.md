# Routing profile: MAX20

Selected when the interview's Q2 ("Which Claude plan is this account on? Pro / Max 5x /
Max 20x") resolves to **Max 20x**. `START-HERE.md` step 5 reads exactly one routing
...
all three routing manifests; this file bolds its own column (**MAX-20X**).
...
`--threshold 1` (one review is one piece of evidence for that phase). For **MAX20**:
`{T2: 2, T3: 3}` — two required phase-gates (spec + audit) at T2, three (spec + audit +
plan) at T3. A T3 ticket with one spec review, one audit review, and one plan review
satisfies the count in full; the number is a phase-gate tally, not a per-phase
repeat-count.
...
## Opus-availability caveat for MAX20's own seats

MAX20's own table cells are not exempt from the honesty rule above. The Orchestrator
seat's "Opus 4.8 (or Sonnet for economy)" and the second-opinion seat's "the stronger
second-opinion model if the account exposes it" are **both** subject to the
silent-substitution behavior described above — a MAX20 account is not guaranteed to
actually run every seat on the model named in its column. If Opus or the stronger
second-opinion model is unavailable, the dispatch falls back silently; treat the
substitution notice, not the table cell, as the source of truth for what actually ran.
